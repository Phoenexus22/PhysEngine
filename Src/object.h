...
/*shapes should be represented by a multidimentional array of vectors (verticies) with each child array being a plane. 
...
void typesizes() //migrate elsewhere
{
	cout << "object: " << sizeof(object) << "\n";
	cout << "vec3: " << sizeof(vec3) << "\n";
	//cout << "medium: " << sizeof(medium);
	cout << "shape: " << sizeof(shape) << "\n";
}

u1 cubePhystest()//tests should migrate elsewhere
{
	object testobject;
...
u1 cubeShapeTest ()//migrate
{
	shape unitCube;
	unitCube.offset = newVec3(0.5, 0.5, 0.5);
	unitCube.vtc = createUnitCube();
	outShape(unitCube);
	return 0;
}
